...
if (Meteor.isServer) {
	describe('Tasks', () => {
		describe('methods', () => {
			...
			it(`can't delete task without a user authenticated`, () => {
			...
			it('can change the status of a task', () => {
				const originalTask = Tasks.collection.findOne(taskId);
			...
			it('can insert new tasks', () => {
				mockMethodCall('tasks.insert', 'new test', {
					context: { userId },
				});

				const tasks = TasksCollection.find({}).fetch();
				assert.equal(tasks.length, 2);
				assert.isTrue(tasks.some((task) => task.text === 'new test'));
			});
		});
	});
}
